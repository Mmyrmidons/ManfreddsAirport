...
<xsl:template match="w">
	...
	<span title="{$lemma} {@pos} {@label}" onmouseover="sumer.gloss('{$lemma}', '{@pos}', '{@label}')"><xsl:call-template name="yevvysDiacriticAnalyzer"/></span>
	<xsl:text> </xsl:text>
</xsl:template>
...
<xsl:template match="l">
	<xsl:choose>
		<xsl:when test="not(@corresp)">
			<div class="persephone" onmouseover="this.style.color = 'lightsalmon';" onmouseout="this.style.color = 'mistyrose';">
